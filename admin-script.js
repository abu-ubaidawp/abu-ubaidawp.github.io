// ===== ADMIN PANEL SCRIPT (Template v2.0) — All bugs fixed =====
...
(async () => {
    ...
    logoutBtn.addEventListener('click', (e) => {
        e.preventDefault();
        signOut(auth).then(() => showToast('লগআউট সফল'));
    });

    // ===== 9. SIDEBAR NAV =====
    const panelTitles = {
        dashboard: 'ড্যাশবোর্ড', profile: 'প্রোফাইল সেটিংস',
        stats: 'পরিসংখ্যান', about: 'পরিচয় / বায়ো',
        news: 'পোস্ট ব্যবস্থাপনা', gallery: 'গ্যালারি',
        ticker: 'টিকার নোটিশ', messages: 'বার্তাসমূহ',
    ...
    function setupUpload(fileInputId, urlInputId, previewId, progressId, progressBarId) {
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            try {
                const url = await uploadToCloudinary(file, pct => { progressBar.style.width = pct + '%'; });
                urlInput.value = url;
    ...
    // Gallery upload (multiple) — BUG FIX: pending URLs stored in closure, not dataset
    ...
    document.getElementById('galleryImageFile').addEventListener('change', async (e) => {
        ...
        progress.classList.add('show');
        pendingGalleryUrls = []; // reset

        for (let i = 0; i < files.length; i++) {
            progressBar.style.width = '0%';
            try {
                const url = await uploadToCloudinary(files[i], pct => { progressBar.style.width = pct + '%'; });
                pendingGalleryUrls.push(url);
            } catch (err) {
                console.error('Upload failed for file', i, err);
            }
        }
        progress.classList.remove('show');
        if (pendingGalleryUrls.length > 0) {
            showToast(`✅ ${pendingGalleryUrls.length}টি ছবি আপলোড সফল!`);
        } else {
            showToast('❌ কোনো ছবি আপলোড হয়নি!');
        }
    });
    ...
    function loadProfile() {
        onValue(ref(db, 'siteSettings'), (snap) => {
            const d = snap.val();
            if (!d) return;
            document.getElementById('profNameBn').value = d.nameBn || '';
            document.getElementById('profNameEn').value = d.nameEn || '';
            document.getElementById('profDesignation').value = d.designation || '';
            document.getElementById('profParty').value = d.party || '';
            document.getElementById('profSlogan').value = d.slogan || '';
            document.getElementById('profHeroPhoto').value = d.heroPhoto || '';
            document.getElementById('profNavIcon').value = d.navIcon || '';
            document.getElementById('profFavicon').value = d.faviconUrl || '';
            document.getElementById('profMeta').value = d.metaDesc || '';
            if (d.heroPhoto) {
                document.getElementById('profHeroPhotoPreview').src = d.heroPhoto;
                document.getElementById('profHeroPhotoPreview').classList.add('show');
            }
            if (d.faviconUrl) {
                document.getElementById('profFaviconPreview').src = d.faviconUrl;
    ...
    document.getElementById('saveProfile').addEventListener('click', async () => {
        ...
        try {
            ...
            showToast('❌ সংরক্ষণ ব্যর্থ!'); console.error(err);
        }
    });
    ...
    function loadStats() {
        onValue(ref(db, 'stats'), (snap) => {
            const d = snap.val();
            if (!d) return;
            document.getElementById('statsYears').value = d.yearsActive || '';
            document.getElementById('statsConstituency').value = d.constituency || '';
            document.getElementById('statsProjects').value = d.projects || '';
            document.getElementById('statsFollowers').value = d.followers || '';
        });
    }
    ...
    document.getElementById('saveStats').addEventListener('click', async () => {
        const data = {
            ...
            yearsLabel: 'বছরের সাংগঠনিক অভিজ্ঞতা'
        };
        try {
    ...
    function loadAbout() {
        onValue(ref(db, 'about'), (snap) => {
            const d = snap.val();
            if (!d) return;
            document.getElementById('aboutBioAdmin').value = d.bio || '';
            document.getElementById('aboutPhotoAdmin').value = d.photo || '';
            document.getElementById('aboutBirthAdmin').value = d.birthDate || '';
            document.getElementById('aboutPlaceAdmin').value = d.birthPlace || '';
            document.getElementById('aboutEduAdmin').value = d.education || '';
            document.getElementById('aboutProfAdmin').value = d.profession || '';
            document.getElementById('aboutPartyAdmin').value = d.party || '';
            document.getElementById('aboutFbAdmin').value = d.facebook || '';
            document.getElementById('aboutYtAdmin').value = d.youtube || '';
            document.getElementById('aboutTwAdmin').value = d.twitter || '';
            if (d.photo) {
                document.getElementById('aboutPhotoPreview').src = d.photo;
    ...
    function renderNewsList() {
        ...
        container.innerHTML = entries.reverse().map(([id, item]) => `
            <div class="list-item">
                ...
                <div class="list-item-actions">
                    <button class="btn-edit" onclick="window._editNews('${id}')"><i class="fas fa-edit"></i></button>
                    <button class="btn-delete" onclick="window._deleteNews('${id}')"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `).join('');
    }
    ...
    document.getElementById('addNewsBtn').addEventListener('click', () => {
        ...
        document.getElementById('newsFormWrap').scrollIntoView({ behavior: 'smooth' });
    });
    ...
    window._deleteNews = function(id) {
        showConfirm('এই পোস্টটি মুছে ফেলতে চান?', async () => {
            try {
                await remove(ref(db, 'news/' + id));
                showToast('✅ পোস্ট মুছে ফেলা হয়েছে!');
            } catch (err) { showToast('❌ মুছতে ব্যর্থ!'); console.error(err); }
        });
    };
    ...
    document.getElementById('saveNews').addEventListener('click', async () => {
        ...
        if (!data.title) { showToast('⚠️ শিরোনাম আবশ্যক!'); return; }
        try {
            if (editId) await set(ref(db, 'news/' + editId), data);
            else await push(ref(db, 'news'), data);
            showToast('✅ পোস্ট সংরক্ষিত হয়েছে!');
            document.getElementById('newsFormWrap').style.display = 'none';
        } catch (err) { showToast('❌ সংরক্ষণ ব্যর্থ!'); console.error(err); }
    });
    ...
    function renderGalleryAdmin() {
        ...
        container.innerHTML = entries.map(([id, item]) => `
            <div class="admin-gallery-item">
                <img src="${item.url}" alt="${item.title || ''}" onerror="this.parentElement.style.display='none'">
                <div class="delete-overlay" onclick="window._deleteGallery('${id}')"><i class="fas fa-trash"></i></div>
            </div>
        `).join('');
    }

    window._deleteGallery = function(id) {
    ...
    document.getElementById('saveGallery').addEventListener('click', async () => {
        ...
        // ✅ BUG FIX: closure variable ব্যবহার করা হচ্ছে, dataset নয়
        if (pendingGalleryUrls.length === 0) {
        ...
        try {
            for (const url of pendingGalleryUrls) {
                await push(ref(db, 'gallery'), { url, title, category, active });
            }
            showToast(`✅ ${pendingGalleryUrls.length}টি ছবি সংরক্ষিত হয়েছে!`);
            // Reset
    ...
    function loadTicker() {
        onValue(ref(db, 'ticker'), (snap) => {
            tickerData = snap.val() || {};
            renderTickerList();
            document.getElementById('dashTicker').textContent = Object.values(tickerData).filter(t => t.active).length;
        });
    }
    ...
    function renderTickerList() {
        ...
        container.innerHTML = entries.map(([id, item]) => `
            <div class="list-item">
                ...
                <div class="list-item-actions">
                    <button class="btn-delete" onclick="window._deleteTicker('${id}')"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `).join('');
    }

    window._deleteTicker = function(id) {
    ...
    function loadMessages() {
        onValue(ref(db, 'messages'), (snap) => {
            ...
            if (entries.length === 0) {
                ...
                dashContainer.innerHTML = entries.slice(0, 5).map(([id, msg]) => `
    ...
    function loadContact() {
        onValue(ref(db, 'contact'), (snap) => {
            const d = snap.val();
            if (!d) return;
            document.getElementById('contactAddrFull').value = d.address || '';
            document.getElementById('contactAddrShort').value = d.addressShort || '';
            document.getElementById('contactPhoneFull').value = d.phone || '';
            document.getElementById('contactPhoneShort').value = d.phoneShort || '';
            document.getElementById('contactEmailAdmin').value = d.email || '';
            document.getElementById('contactHoursAdmin').value = d.hours || '';
            document.getElementById('contactMapAdmin').value = d.mapUrl || '';
            document.getElementById('contactWaAdmin').value = d.whatsapp || '';
            document.getElementById('contactCallAdmin').value = d.callNumber || '';
        });
    }
